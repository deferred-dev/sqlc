...
func Cmd(n ast.Node, name, cmd string) error {
	...
	if !(cmd == metadata.CmdMany || cmd == metadata.CmdOne || cmd == metadata.CmdBatchMany || cmd == metadata.CmdBatchOne || cmd == metadata.CmdIter || cmd == metadata.CmdMap || cmd == metadata.CmdSet) {
		return nil
	}
	var list *ast.List
	switch stmt := n.(type) {
